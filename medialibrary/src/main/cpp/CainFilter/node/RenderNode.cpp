...
void RenderNode::logTime() {
    char strff[21];
    memset(strff, 0, sizeof(strff));
    sprintf(strff, "%d", this->effectStartTime);
    __android_log_print(ANDROID_LOG_ERROR, "!!!", "%s", strff);
}
...
bool RenderNode::contains() {
    return timeStamp >= effectStartTime && timeStamp <= effectEndTime;
}

void RenderNode::setTime(long start, long end) {
    effectStartTime = start;
    effectEndTime = end;
}

int
RenderNode::drawFrameBuffer(GLuint texture, const float *vertices, const float *textureVertices) {
